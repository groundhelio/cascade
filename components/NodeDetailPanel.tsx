...
const NodeDetailPanel: React.FC<NodeDetailPanelProps> = ({ node, onClose, onExpand, onRefresh, isExpanding, selectedCountry, breakpoint }) => {
  ...
  const headerTextSize = isMobile ? 'text-xl' : 'text-2xl';
  const sectionTextSize = isMobile ? 'text-base' : 'text-lg';

  return (
    <div className={`h-full w-full bg-white ${containerPadding} flex flex-col overflow-y-auto smooth-scroll`}>
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        <h2 className={`${headerTextSize} font-bold text-gray-900 pr-2`}>{node.label}</h2>
        <button 
          onClick={onClose} 
          className="text-gray-500 hover:text-gray-900 transition-colors flex-shrink-0 ml-2 min-h-[44px] min-w-[44px] flex items-center justify-center"
          title="Close panel"
        >
      ...
      <div className="mb-4 sm:mb-6">
        <h3 className={`${sectionTextSize} font-semibold text-gray-900 mb-2 border-b border-gray-200 pb-1`}>Cascading Severity Radar</h3>
        {node.severityScores === undefined || node.severityScores === null ? (
             <div className="flex justify-center items-center h-48">
                <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-gray-900"></div>
            </div>
        ) : (
            <SeverityRadarView scores={node.severityScores} breakpoint={breakpoint} />
        )}
      </div>
            ...
            <ul className="space-y-2 sm:space-y-3 list-inside">
              {node.memory.reflections.map((reflection, index) => (
                <li key={index} className="flex items-start">
                  ...
                  <p className="text-gray-700 flex-1 text-sm sm:text-base">{reflection}</p>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="mt-6 sm:mt-8 space-y-3">
        ...
        <button
          ...
          className="w-full bg-gray-900 hover:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50 min-h-[44px]"
        >
      ...
      <div className="mt-4 sm:mt-6 border-t border-gray-200 pt-4">
        <button
          ...
          className="w-full flex items-center justify-between text-sm text-gray-600 hover:text-gray-900 transition-colors py-2 min-h-[44px]"
        >
        ...
        {showColorLegend && (
          <div className="mt-3 space-y-2 sm:space-y-3 text-sm">
            {/* Root Node */}
            <div className="bg-red-50 border border-red-200 rounded-lg p-2 sm:p-3">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: '#e11d48' }}></div>
                <div className="flex-1">
                  <div className="font-bold text-gray-900 text-sm">Root Event (Crisis)</div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 sm:p-3">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex-shrink-0 mt-0.5 bg-gradient-to-br from-blue-500 to-purple-500"></div>
                <div className="flex-1">
                  <div className="font-bold text-gray-900 text-sm">Primary Effects (Neutral/Mixed)</div>
                  <div className="text-gray-700 text-xs mt-1">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 sm:p-3">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: '#b45309' }}></div>
                <div className="flex-1">
                  <div className="font-bold text-gray-900 text-sm">Consequences (Negative)</div>
                  <div className="text-gray-700 text-xs mt-1">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-2 sm:p-3">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: '#10b981' }}></div>
                <div className="flex-1">
                  <div className="font-bold text-gray-900 text-sm">Responses (Positive)</div>
                  <div className="text-gray-700 text-xs mt-1">
